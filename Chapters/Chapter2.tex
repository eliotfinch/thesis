...
% Sparked many other papers, some exploiting this discovery of the importance of overtones, others arguing against their physical validity.
% Even at the time of writing no consensus has been reached...
...
The difficulties of defining a suitable start time and some surprising data analysis consequences of this were discussed in Ref.~\cite{Thrane:2017lqn}. 
...
Typically, the start time is referred to the maxima of some time-dependent quantity (e.g.\ the 22 mode of the strain, the modulus of the Weyl scalar, or the total GW luminosity; these quantities peak at times that typically differ by a few tens of $M$) and typical choices were $10M$ to $20M$ after the peak (although, see Ref.~\cite{Berti:2007fi} where a range of different start times are explored). 
Several studies caution against starting too early \cite{JimenezForteza:2020cve, Bhagwat:2019dtm}.

More recently, Ref.~\cite{Giesler:2019uxc} (see also Ref.~\cite{Ota:2019bzl}) looked to allow the ringdown to start significantly earlier by including multiple overtones. 
In particular, Ref.~\cite{Giesler:2019uxc} found that by including up to seven overtones the ringdown analysis can be started as early as the peak in the 22 mode of the strain. 
...
Even more surprising, in Ref.~\cite{Dhani:2020nik} this approach was extended by the inclusion of mirror modes along with overtones (thereby doubling the number of QNMs) and it was found that it was possible to start the ringdown analysis even earlier (up to $10M$ before the peak). 
...
A ringdown model with overtones was successfully applied to GW150914 in Ref.~\cite{Isi:2019aib} to extract the fundamental QNM and the first overtone from the noisy data, and more recently to events in GWTC-2 \cite{LIGOScientific:2020tif}. 
...
We note that some work on precessing systems has been done in the extreme mass ratio limit, see Refs.~\cite{Hughes:2019zmt,Lim:2019xrb}. 
...
The primary aim of this paper is to address this question by systematically extending the analyses of Refs.~\cite{Giesler:2019uxc, Dhani:2020nik} to a large number of precessing BBH simulations from the SXS catalog \cite{Boyle:2019kee, Mroue:2013xna}. 
...
Numerical relativity expands the gravitational-wave (GW) strain in the basis of (spin-weighted) spherical harmonics
\begin{equation}\label{eq:spherical_expansion}
    rh = M \sum_{\ell m} h_{\ell m}(t) {}_{-2}Y_{\ell m}(\Omega),
\end{equation}
where we suppress the sum limits for clarity, and $\Omega$ is used as shorthand for the angles $\theta$, $\phi$.
We will assume the above has been rotated such that the remnant black-hole (BH) spin vector is aligned with the $z$-axis.

Perturbation theory expands the GW strain in the basis of the (spin-weighted) spheroidal harmonics
\begin{equation}\label{eq:spheroidal_expansion}
    rh = M \sum_{\ell m n} \left[ C_{\ell m n} e^{-i \omega_{\ell m n} t} {}_{-2}S_{\ell m n}(\Omega) + C'_{\ell m n} e^{-i \omega'_{\ell m n} t} {}_{-2}S'_{\ell m n}(\Omega) \right],
\end{equation}
where we have defined $t$ such that the ringdown start time $t_0 = 0$.
Here, $C_{\ell m n}$ are complex coefficients (containing an amplitude and a phase), $\omega_{\ell m n} = 2\pi f_{\ell m n} - i/\tau_{\ell m n}$ are the complex quasinormal mode frequencies (which are functions of the remnant BH mass $M_f$ and spin $\chi_f$), and ${}_{-2}S_{\ell m n}(\Omega) = {}_{-2}S_{\ell m}(\Omega; a\omega_{\ell m n})$ are the spheroidal harmonics.
...
These mirror modes are related to the regular modes via $\omega'_{\ell m n} = -\omega^*_{\ell -m n}$.
...
\begin{align}\label{eq:hlm_model}
...
Given some spherical-harmonic modes $h_{\ell m}$, we can turn Eq.~\ref{eq:hlm_model} into a least-squares fitting problem to find the best-fit complex coefficients $C_{\ell m n}$ and $C'_{\ell m n}$ (this assumes the complex frequencies and mixing coefficients are also given). 
First, we write Eq.~\ref{eq:hlm_model} as a matrix equation. 
Note that in the above I have separated the regular and mirror modes to show explicitly how to deal with the mirror modes (i.e. how to obtain their frequencies and mixing coefficients from the regular frequencies and mixing coefficients). 
I will now drop this distinction for clarity.

\subsection{Single-mode fit}

For simplicity, I first consider the case when we want to model a single spherical mode (for example, the $h_{22}$ mode). We write
\begin{equation}\label{eq:hlm_matrix_single}
    \vb*{h}_{\ell m} = \vb*{a}_{\ell m} \vdot \vb*{C}
\end{equation}
where $\vb*{h}_{\ell m} = \left[h_{\ell m}(u_0),\ h_{\ell m}(u_1), \ldots,\ h_{\ell m}(u_{K-1}) \right]$ is the waveform data evaluated at a total of $K$ times labeled by $u_k$. 
In general we will have 
\begin{equation}
    t_0 \neq u_0, \quad u_{K-1} \neq t_0 + T
\end{equation}
where $t_0$ is our chosen ringdown start time, and $T$ determines the length of our ringdown model. 
So, we will need a method for choosing $u_0$ (e.g. it could be the first value after $t_0$, or the closest value to $t_0$).

The matrix $\vb*{a}_{\ell m}$ is the coefficient matrix, which is where the choice of QNM content in our model enters. 
It has the form
\begin{equation}
    \vb*{a}_{\ell m} = 
    \begin{pmatrix}
    e^{-i \omega_0 (u_0 - t_0)} \mu_{\ell m,0} & e^{-i \omega_1 (u_0 - t_0)} \mu_{\ell m,1} & \cdots & e^{-i \omega_{J-1} (u_0 - t_0)} \mu_{\ell m,J-1} \\ 
    e^{-i \omega_0 (u_1 - t_0)} \mu_{\ell m,0} & e^{-i \omega_1 (u_1 - t_0)} \mu_{\ell m,1} & \cdots & e^{-i \omega_{J-1} (u_1 - t_0)} \mu_{\ell m,J-1} \\ 
    \vdots & \vdots & \ddots & \vdots \\
    e^{-i \omega_0 (u_{K-1} - t_0)} \mu_{\ell m,0} & e^{-i \omega_1 (u_{K-1} - t_0)} \mu_{\ell m,1} & \cdots & e^{-i \omega_{J-1} (u_{K-1} - t_0)} \mu_{\ell m,J-1}
    \end{pmatrix}
\end{equation}
...
To reiterate, the QNM content in our model can consist of any regular or mirror modes, as long as the correct frequencies and mixing coefficients are used in the above matrix (I have just used $\omega_j$ and $\mu_{\ell m, j}$ as generic terms). 
...
\begin{equation}
    ...
    \end{bmatrix}
\end{equation}
and $I$ is the number of spherical-harmonic modes to include in the fit (I have suppressed the two spherical-harmonic indices for clarity). So, $\vb*{h}$ is a vector of length $I \times K$.
Similarly
\begin{equation}
    \vb*{a} = 
    \begin{bmatrix}
...
In \cite{Giesler:2019uxc}, the efficacy of this model for $\ell=m=2$ was demonstrated by performing least squares fits to the $h_{22}(t)$ mode for a selection of aligned-spin SXS simulations. The authors note that this was also verified for other values of $(\ell,m)$.
