...
The characteristic oscillations of the remnant BH are called quasinormal modes (QNMs), so-called because, unlike normal modes, they decay over time (for reviews on the subject, see Refs.~\cite{Kokkotas:1999bd, Nollert:1999ji, Berti:2009kk}).
...
The term QNM coined in Ref.~\cite{Press:1971wr}.

Any system which loses energy 
BH spacetimes are intrinsically dissipative due to the presence of an event horizon.
...
\begin{itemize}
    ...
    \item Teukolsky obtains separable wave equations for electromagnetic and gravitational perturbations of a rotating BH~\cite{Teukolsky:1972my}.
\end{itemize}
}

Vishveshwara found, via numerical studies, that BHs have a characteristic ``sound''~\cite{Vishveshwara:1970zz}. 
They employed the perturbation techniques developed by Regge and Wheeler. 
...
% explain kg is a massless wave equation, qualitatively similar to other equations
% maybe pull all mentions of scalar field from first paragraph and put here. 
\begin{equation}
    \nabla_\mu \nabla^\mu \psi = 0.
\end{equation}
...
For Schwarzschild we have \comment{maybe don't need both} $g = -r^4 \sin^2{\theta} \implies \sqrt{-g} = r^2 \sin{\theta}$.
Evaluating, we get
\begin{multline}
...
\end{multline}
% We use scalar Ys here, GW use spin-weighted, refer to later equations?
...
% Refer to FT equation used elsewhere in thesis for convention? Or introduce here
...
\end{equation}
% Really polar also obeys the same form, maybe make that clearer?
...
We must specify boundary conditions to % find the QNM solutions. complete the problem. 
...
Instead, the potential vanishes for $r_* \rightarrow \pm \infty$, which implies we need to look for purely outgoing plane waves: %be more precise by outgoing. outgoing to inf, ingoing to -inf
\begin{alignat}{2}
...
% now have a well-posed problem. just a problem of solving. can be done using above methods. solutions presented in 1.4. first we gain some more physical intuition. 
...
The Schwarzchild spectrum, which depends only on the BH mass and has no $m$ dependence, can be recovered by taking the branch point of each $(\ell, n)$ group.
