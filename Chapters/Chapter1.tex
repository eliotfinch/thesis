...
This is somewhat analogous to the expansion of the Universe, which causes distant galaxies (the test masses) to appear to recede from us due to the expansion of space.
The LIGO and Virgo GW detectors employ interferometry to measure the motion of test masses to extreme precision.
In this setup the mirrors at the end of the interferometer arms act as the test masses (they are hung as pendulums to isolate them from the environment), and changes in their position can be determined via the light travel time of the interferometer laser.
The ultimate detector output is a timeseries of a dimensionless scalar quantity called the strain, $h$, which quantifies the changes in position of the test masses (which can also be thought of as changes to the interferometer arm lengths).
...
The GW signal from a BBH merger can be split into three regions: the inspiral, merger, and ringdown.
...
The GW signal from these three regimes is shown in Fig.~\ref{fig:ch1:td_waveform}.
We use geometric units ($G = c = 1$) to express time and (scaled) strain in units of the total binary mass, $M$. 
Specifically, plotted is the GW signal projected onto the LIGO Hanford detector, $h$, multiplied by the distance to the source, $r$ (but note that in reality the Hanford data stream will be noise dominated, see Fig.~\ref{fig:ch1:fd_waveform}).
In geometric units, we can convert between lengths, masses, and times with appropriate combinations of $G$ and $c$, which means time$/M$ and $rh/M$ can be made dimensionless quantities. 
...
This can be compared with what was measured for GW150914 (see Fig.~1 in \cite{LIGOScientific:2016aoc}), and we see these numbers are consistent.
...
% We leave the factors of $G$ and $c$ in for clarity.
...
% A note comparing phenom models to surrogates... The original motivation of the Phenom approach was to produce an approximate and efficient waveform family suitable for GW searches (the models are written as closed-form analytic expressions in the frequency domain)
...
% Drop the radial dependance... 
The reason for this is because the $r$ dependence of the waveform goes predominately like $r^{-1}$, so the radial dependence can be removed by multiplying through by $r$. For clarity we will drop the $r$ factor in the equations below and have the $r^{-1}$ scaling implied (just keep in mind when we refer to the GW signal $h$ or a mode $h_{\ell m}$, we really mean $rh$ and $rh_{\ell m}$). 
...
\begin{figure}[ht!]
	...
	\caption[The gravitational-wave ringdown signal]{ 
		The ringdown waveform from a BBH, fitted with a simple damped sinusoid. The waveform is a numerical relativity simulation from the SXS catalog.}
	\label{fig:ch1:rd_waveform}
\end{figure}

% The model loses validity at early times, but the inclusion of additional ringdown modes in the model (in particular, overtones, has been proposed as a method of extending the linear ringdown description to earlier times (but controversy...)
