...
To understand the response of a interferometer to incoming GWs, consider a setup where the interferometer beam-splitter is at the origin and the mirrors are at the coordinates $(L_x,0)$ and $(0,L_y)$. \comment{Figure?}
A GW incoming from the positive $z$ direction will influence the propagation of light in the interferometer arms. 
In the proper detector frame, the GW acts to displace the mirrors and can be intuitively described in terms of Newtonian forces. 
However, GWs take on a simple description when working in the transverse-traceless (TT) frame.
...
\comment{For example, taking the plus, its effect on the interferoemter arm is...}
...
% (just keep in mind when we refer to the GW signal $h$ or a mode $h_{\ell m}$, we really mean $rh$ and $rh_{\ell m}$). 
