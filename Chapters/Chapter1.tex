...
\end{equation}
were $\rho(\vb*{x})$ is the mass density and $\vb*{x} = (x_1, x_2, x_3) = (x,y,z)$ with some suitable choice of origin.
...
Eq.~\ref{eq:time_to_merger} is valid in the quasi-circular orbit regime; it can be derived by equating the time derivative of the orbital energy of a binary system (\comment{give expression?}) to (negative) the GW luminosity Eq.~\ref{ch1:eq:gw_luminosity}, where the quadrupolar moment is most easily calculated using the equivalent one-body problem with a reduced mass.
