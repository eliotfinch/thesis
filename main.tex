...
Secondly, we develop a novel frequency-domain ringdown analysis method for use with real data, applying it to simulated gravitational-wave signals and to GW150914.
Our method avoids some of the problems associated with conventional ringdown-only analyses.
Applying the method to GW150914, we contribute to the uncertain significance with which a ringdown overtone can be found in the data.
\end{abstract}
...
Chapter~\ref{Chapter1} is an introductory chapter and contains no original work.
...
Our approach avoids the issues of spectral leakage that would normally be expected (associated with the abrupt start of the ringdown) by modelling the inspiral and merger parts of the signal using a flexible sum of sine-Gaussian wavelets truncated at the onset of the ringdown. 
