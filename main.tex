...
% which is more involved and requires an understanding of the BMS transformation properties of the waveform.
% , which is more involved but allows the signal to be transformed to the superrest frame~\cite{MaganaZertuche:2021syq}.
% Existing NR waveforms typically extrapolate to find the GW signal at large distances from the source; this is known to perform poorly in the ringdown [KEEFE]. The next step in this field will require working instead with CCE waveforms which is more involved and requires an understanding of the BMS transformation properties of the waveform.
% In collaboration with Leo Stein and Keefe Mitman, I have started using the \texttt{scri}~\cite{boyle_michael_2022_7222697} package to transform CCE waveforms to the superrest frame for use in ringdown studies.
